...
impl GenericPhysicalDevice for LinuxPhysicalDevice {
    fn create_device(
        ...
        if self.name == "virtio_gpu" || self.name == "virtgpu" || pci_info.vendor_id == 0x1af4 {
            return Ok(Arc::new(crate::sys::linux::VirtGpu::new(
                physical_device.clone(),
            )?));
        }

        let device: Arc<dyn Device> = match pci_info.vendor_id {
            0x1af4 => Arc::new(crate::sys::linux::VirtGpu::new(physical_device.clone())?),
